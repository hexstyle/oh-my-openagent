...
- `script/install-local-opencode-fork.sh --reset` is the supported clean-install path. It builds the fork, syncs the managed config, rewrites the live host plugin entry to `file://<repo-root>`, installs the managed runtime plugin set, imports `Codex` OAuth, and finishes with `script/verify-local-opencode-install.ts`.
...
- That runtime-fallback policy now keeps transient same-model retries alive for up to 4 hours, increases retry spacing over time, caps that spacing at 5 minutes, then only falls through to the paid -> spark -> free chain after the retry window expires.
...
- `opencode-supermemory` is intentionally not part of the managed baseline because it overlaps with the fork compaction stack and is treated as a manual integration.
