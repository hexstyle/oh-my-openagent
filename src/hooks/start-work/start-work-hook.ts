...
function detectCIFastPath(planPath: string, projectDir: string): CIFastPathResult {
  ...
  try {
    ...
    const block = [
      ...
      "Use the current session id already provided by `/start-work`; do NOT call `session_list`, do NOT query `printenv SESSION_ID`, and do NOT spend a turn rediscovering runtime metadata that is already in this prompt.",
